...
MIN_COV_VAL = 1e-5
# def get_kmeans_emission_init(self, sequence, K):
#     assignments = KMeans(n_clusters=K, random_state=0).fit_predict(sequence)
#     means = []
#     covs = []
#     for k in range(K):
...
#     return means, covs
#
# def calculateLikelihood(sequence, K):
#     means, covs = get_kmeans_emission_init(sequence, K)
#     return [multivariate_normal(mean=means[k], cov=covs[k], allow_singular=True) for k in range(K)]
#
# likelihood = calculateLikelihood(sequence, K)
#
# def log_likelihood(rolls):
#     # rolls: shape [1, n_rolls]
#     log_prob = []
#     for roll in rolls:
#         log_prob.append([likelihood[k].log_prob(roll) for k in range(len(likelihood))])

class CRF(nn.Module):
    '''
    Modified CRF, log_likelihood is a function
    ...
    def _compute_likelihood_denominator(self, loglikelihoods):
        ...
        for roll in loglikelihoods[1:]:
            ...
            for next_state in range(self.n_states):
                ...
                alpha_t.append(self.log_sum_exp(alpha_t_next_state).view(1, -1))
    ...
    def _viterbi_algorithm(self, loglikelihoods):
        ...
        for roll in loglikelihoods[1:]:
            ...
            for next_state in range(self.n_states):
                ...
                next_delta.append(score.view(1, -1))
                # next_delta.append(self.log_sum_exp(score))
    ...
    def _get_kmeans_emission_init(self, sequence, K):
        assignments = KMeans(n_clusters=K, random_state=0).fit_predict(sequence)
        means = []
        covs = []
        for k in range(K):
    ...
    def update_log_likelihood(self, rolls):
        means, covs = self._get_kmeans_emiision_init(rolls, self.n_states)
        distribution = [multivariate_normal(mean=means[k], cov=covs[k], allow_singular=True) for k in range(K)]
        log_prob = []
        for roll in rolls:
            log_prob.append([distribution[k].log_prob(roll) for k in range(len(distribution))])
        self.loglikelihood = np.array(log_prob)
