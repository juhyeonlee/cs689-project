...
class GaussianHMM(object):
    ...
    def __init__(self, number_of_states, sequence, regularization_mode=0):
        ...
        self.emission_density_objs = [
            multivariate_normal(mean=emission_means[k], cov=emission_covariances[k], allow_singular=True) for k in
    ...
    def forward(self, sequence):
        ...
        for t in range(1, T):
            for k in range(K):
                alpha_table[t][k] = np.dot(alpha_table[t - 1, :], self.trans_probs[:, k]) * self.get_emission_prob(k,
                                                                                                                   x[t])
            if not np.all(alpha_table[t]):
                for k in range(K):
                    alpha_table[t][k] = np.dot(alpha_table[t - 1, :], self.trans_probs[:, k]) * self.get_emission_prob(
                        k, x[t]) + MIN_ALPHA_BETA
    ...
    def backward(self, sequence):
        ...
        for t in range(T - 2, -1, -1):
            for k in range(K):
                beta_table[t][k] = np.sum(
                    [beta_table[t + 1][j] * self.trans_probs[k][j] * self.get_emission_prob(j, x[t + 1]) for j in
                     range(K)])
            if not np.all(beta_table[t]):
                for k in range(K):
                    beta_table[t][k] = np.sum(
                        [beta_table[t + 1][j] * self.trans_probs[k][j] * self.get_emission_prob(j, x[t + 1]) for j in
                         range(K)]) + MIN_ALPHA_BETA
    ...
    def compute_xi_table(self, sequence):
        ...
        for t in range(1, T):
            ...
            for i in range(K):
                for j in range(K):
                    xi_table[t][i][j] = (1. / sf) * self.alpha_table[t - 1][i] * self.trans_probs[i][
                        j] * self.get_emission_prob(j, point) * self.beta_table[t][j]
    ...
    def update_transition_probs_inertial_regularized(self, sequence, zeta=1):
        ...
        K = self.num_states
        v = (T - 1) ** zeta
        amped_vals = [[None for k in range(K)]] + [
            [(self.gamma_table[t - 1][k] - self.xi_table[t][k][k]) * v for k in range(K)] for t in range(1, T)]
        for j in range(K):
            denom = np.sum(
                [self.xi_table[t][j][i] + (amped_vals[t][i] if i == j else 0.0) for i in range(K) for t in range(1, T)])
            for k in range(K):
                self.trans_probs[j][k] = np.sum(
                    [self.xi_table[t][j][k] + (amped_vals[t][k] if j == k else 0.0) for t in range(1, T)]) / denom
        self.smooth_transition_probs()
    ...
    def learn(self, sequence, zeta=None, init=False, only_final_ll=False):
        ...
        if init:
            ...
            self.emission_density_objs = [multivariate_normal(mean=means[k], cov=covs[k], allow_singular=True) for k in
                                          range(K)]
            start_probs, transition_probs = self.get_random_start_and_trans_probs()
            self.start_probs = start_probs
            self.trans_probs = transition_probs
        while True:
            ...
            if abs(self.log_likelihood - old_ll) < epsilon or iterations > MAX_ITER:
                print
                "Final Log Likelihood:", self.log_likelihood
                return
            else:
